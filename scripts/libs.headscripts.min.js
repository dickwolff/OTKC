window.Modernizr=function(aw,av,au){function S(b){am.cssText=b
}function R(d,c){return S(ai.join(d+";")+(c||""))
}function Q(d,c){return typeof d===c
}function P(d,c){return !!~(""+d).indexOf(c)
}function O(f,c){for(var h in f){var g=f[h];
if(!P(g,"-")&&am[g]!==au){return c=="pfx"?g:!0
}}return !1
}function N(g,c,j){for(var i in g){var h=c[g[i]];
if(h!==au){return j===!1?g[i]:Q(h,"function")?h.bind(j||c):h
}}return !1
}function M(g,f,j){var i=g.charAt(0).toUpperCase()+g.slice(1),h=(g+" "+ag.join(i+" ")+i).split(" ");
return Q(f,"string")||Q(f,"undefined")?O(h,f):(h=(g+" "+af.join(i+" ")+i).split(" "),N(h,f,j))
}var at="2.8.3",ar={},aq=!0,ap=av.documentElement,ao="modernizr",an=av.createElement(ao),am=an.style,al,ak=":)",aj={}.toString,ai=" -webkit- -moz- -o- -ms- ".split(" "),ah="Webkit Moz O ms",ag=ah.split(" "),af=ah.toLowerCase().split(" "),ae={svg:"http://www.w3.org/2000/svg"},ad={},ac={},ab={},aa=[],Z=aa.slice,Y,X=function(v,u,t,s){var r,q,p,o,h=av.createElement("div"),g=av.body,b=g||av.createElement("body");
...
},W=function(a){var f=aw.matchMedia||aw.msMatchMedia;
if(f){return f(a)&&f(a).matches||!1
}var e;
return X("@media "+a+" { #"+ao+" { position: absolute; } }",function(c){e=(aw.getComputedStyle?getComputedStyle(c,null):c.currentStyle)["position"]=="absolute"
}),e
},V=function(){function c(h,g){g=g||av.createElement(b[h]||"div"),h="on"+h;
var a=h in g;
return a||(g.setAttribute||(g=av.createElement("div")),g.setAttribute&&g.removeAttribute&&(g.setAttribute(h,""),a=Q(g[h],"function"),Q(g[h],"undefined")||(g[h]=au),g.removeAttribute(h))),g=null,a
}var b={select:"input",change:"input",submit:"form",reset:"form",error:"img",load:"img",abort:"img"};
return c
}(),U={}.hasOwnProperty,T;
!Q(U,"undefined")&&!Q(U.call,"undefined")?T=function(d,c){return U.call(d,c)
}:T=function(d,c){return c in d&&Q(d.constructor.prototype[c],"undefined")
...
return f
}),ad.flexbox=function(){return M("flexWrap")
},ad.flexboxlegacy=function(){return M("boxDirection")
},ad.canvas=function(){var b=av.createElement("canvas");
return !!b.getContext&&!!b.getContext("2d")
},ad.canvastext=function(){return !!ar.canvas&&!!Q(av.createElement("canvas").getContext("2d").fillText,"function")
...
},ad.hashchange=function(){return V("hashchange",aw)&&(av.documentMode===au||av.documentMode>7)
...
},ad.rgba=function(){return S("background-color:rgba(150,255,150,.5)"),P(am.backgroundColor,"rgba")
},ad.hsla=function(){return S("background-color:hsla(120,40%,100%,.5)"),P(am.backgroundColor,"rgba")||P(am.backgroundColor,"hsla")
},ad.multiplebgs=function(){return S("background:url(https://),url(https://),red url(https://)"),/(url\s*\(.*?){3}/.test(am.background)
},ad.backgroundsize=function(){return M("backgroundSize")
},ad.borderimage=function(){return M("borderImage")
},ad.borderradius=function(){return M("borderRadius")
},ad.boxshadow=function(){return M("boxShadow")
},ad.textshadow=function(){return av.createElement("div").style.textShadow===""
},ad.opacity=function(){return R("opacity:.55"),/^0.55$/.test(am.opacity)
},ad.cssanimations=function(){return M("animationName")
},ad.csscolumns=function(){return M("columnCount")
...
return S((e+"-webkit- ".split(" ").join(d+e)+ai.join(f+e)).slice(0,-e.length)),P(am.backgroundImage,"gradient")
},ad.cssreflections=function(){return M("boxReflect")
},ad.csstransforms=function(){return !!M("transform")
},ad.csstransforms3d=function(){var b=!!M("perspective");
...
},ad.csstransitions=function(){return M("transition")
...
for(var L in ad){T(ad,L)&&(Y=L.toLowerCase(),ar[Y]=ad[L](),aa.push((ar[Y]?"":"no-")+Y))
}return ar.addTest=function(e,c){if(typeof e=="object"){for(var f in e){T(e,f)&&ar.addTest(f,e[f])
}}else{e=e.toLowerCase();
if(ar[e]!==au){return ar
}c=typeof c=="function"?c():c,typeof aq!="undefined"&&aq&&(ap.className+=" "+(c?"":"no-")+e),ar[e]=c
}return ar
},S(""),an=al=null,function(ax,K){function A(f,e){var h=f.createElement("p"),g=f.getElementsByTagName("head")[0]||f.documentElement;
...
ax.html5=t,u(K)
}(this,av),ar._version=at,ar._prefixes=ai,ar._domPrefixes=af,ar._cssomPrefixes=ag,ar.mq=W,ar.hasEvent=V,ar.testProp=function(b){return O([b])
},ar.testAllProps=M,ar.testStyles=X,ar.prefixed=function(e,d,f){return d?M(e,d,f):M(e,"pfx")
...
},Modernizr.addTest("cssvhunit",function(){var b;
return Modernizr.testStyles("#modernizr { height: 50vh; }",function(a,h){var g=parseInt(window.innerHeight/2,10),f=parseInt((window.getComputedStyle?getComputedStyle(a,null):a.currentStyle).height,10);
b=f==g
}),b
});
window.Modernizr=(function(b,a,c,d){c.addTest("picture","HTMLPictureElement" in b);
c.addTest("volume",function(){var e=a.createElement("audio");
e.volume=0.5;
return c.audio&&e.volume===0.5
});
if(!c.picture){a.createElement("picture")
}return c
...
playback timings (ms):
  ...
  esindex: 0.011
  captures_list: 165.457
  CDXLines.iter: 17.527 (3)
  PetaboxLoader3.datanode: 138.159 (5)
  exclusion.robots: 0.286
  exclusion.robots.policy: 0.27
  RedisCDXSource: 47.752
  PetaboxLoader3.resolve: 75.989 (2)
  load_resource: 165.141
*/